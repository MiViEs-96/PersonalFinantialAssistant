...
### Funzioni Chiave:
- **`inject_translations()`**: Un `context_processor` che rende disponibile la funzione `_()` in tutti i template HTML. Cerca le traduzioni prima in `custom_translations.json` e poi in `translations.py`.
...
- **`add_category()`**: Salva una nuova categoria in `categories.json` (usando il nome inglese come chiave) e memorizza le traduzioni fornite dall'utente in `custom_translations.json`.
...
- **`delete_category()`**: Verifica che una categoria non sia usata in nessuna transazione prima di eliminarla dai file JSON.
...
6. Il server salva le traduzioni in `custom_translations.json`.
